...
// 参考： https://wangpengcheng.github.io/2019/07/03/cplusplus_concurrency_in_action_01/
...
如果需要自己指定线程销毁，必须在std::thread对象销毁之前决定
...
// 在从 char*到std::string类型转换的过程中，函数很有可能在转化成功之前崩溃
// 但是 std::thread 的构造函数会复制提供的变量，就只复制了没有转换成期望类型的字符串字面值，最终造成程序崩溃
...
// 注意:
// 当想要在线程构造函数中传递参数为引用参数，使得变量在线程中进行更改的时候，需要使用std::ref将参数转换成为引用的形式，避免其在构造的过程中使用默认拷贝
// 如std::thread t(update_data_for_widget,w,std::ref(data));;这样data就在线程前后发生了改变。
...
// 2.3 转移线程所有权
// 为了保证线程控制句柄的有效性，std::thread支持使用std:move()函数实现函数的转移，但是线程的拷贝构造和赋值操作符决定了，
// 它在进行复制时会先使用std::terminate()强制结束原来已有的线程，再接受新句柄；即说明：不能通过赋一个新值给std::thread 对象的方式来“丢弃”一个线程。
...
//t2线程句柄，交给t3 
t3=std::move(t2);
//t1执行std::terminate()终止程序继续运行，保证与std::thread的析构函数行为一致，需要再:
t1=std::move(t3);
...
// std::thread::get_id()函数直接来获取。当没有绑定线程时，函数返回std::thread::type
// 在当前线程中调用std::this_thread::get_id()获取当前的线程标识符
...
// ,使用事务的方式去处理数据结构的更新(这里的”处理”就如同对数据库进行更新一样)。所需的一些数据和读取都存储在事务日志中,然后将之前的操作合为一步,再进行提交。
// 在被另外一个线程修改后，提交就无法进行。这成为“软件事务内存”。但是C+++中没有对STM进行支持。
// C++中保护共享数据的最基本方式是使用 C++标准库提供的互斥量


// 3.2 使用互斥两保护共享数据
// 3.2.1 C++中使用互斥量
// c++中通过使用std::mutex创建互斥量，通过调用成员函数lock()/unlock()进行上锁/解锁。同时可以使用std::lock_guard()，在构造的时候提供已锁的互斥，
// 并在析构的时候进行解锁，从而保证总是会被正确的解锁。 使用示例：
...
void add_to_list(int new_value)
{
    ...
    std::lock_guard<std::mutex> guard(some_mutex);    //添加数据
...
// 3.2.2 精心组织代码来保护共享数据
// 针对返回指针或者引用会使得成员变量失去保护。这样互斥变量形同虚设。但是这些只要你确保没有在使用过程中显式调用就没有问题，
// 但是更危险的是 将一个保护数据作为运行时参数 例如下面，无意中使用了保护数据的引用
...
// 是std::lock_guard。这东西是干什么的呢？它是与mutex配合使用，把锁放到lock_guard中时，mutex自动上锁，lock_guard析构时，同时把mutex解锁
...
// std::unique_lock可以根据传入的第二个参数的不同，对析构函数进行管理，使用std::defer_lock作为参数的时候，表明互斥量应该保持解锁状态，方便被其它线程加锁。
// 但是 std::unique_lock会占用比较多的空间,并且比std::lock_guard稍慢一些。并且std::unique_lock实例不带互斥量：信息一倍存储，且被更新。

// std::lock() 和 std::unique_lock的使用
