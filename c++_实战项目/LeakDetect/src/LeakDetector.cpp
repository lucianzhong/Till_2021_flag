...
// 重载operator new/new[ ] 与 operator delete/delete[ ], 并借助双向链表结构(带头节点)我们自己手动管理内存
...
// 创建一个头结点, 它的前后指针均初始化为指向自己(插入、删除双向链表中结点 和 _LeakDetector( )函数中遍历双向链表时,
// 这样初始化的作用就体现出来了)。使用静态变量使其只在本文件内有效
...
void* AllocateMemory( size_t size, bool array, char* file, size_t line){
    ...
    // 注意, 此处为size而不是newSize. 因为我们管理记录的是 new申请的内存, 验证它是否未释放, 存在内存泄漏问题. 
    // 申请 newSize的内存(为 MemoryList结点多申请出的内存), 只是为了实现手动管理内存所必须, 这个内存我们一定会释放, 
    // 不需关注. 所以保存 时用size而不是newSize
	newElem->_size = size;		
	...
	// 更新未释放的内存数
	// 我们管理的只是 new申请的内存. 为memoryListHead结点多申请的内存,和为保存文件信息多申请内存无关, 
    // 这些内存我们一定会释放, 所以这里只记录size
...
// 重载delete/delete[]运算符  // noexcept修饰的函数不会抛出异常
...
// 最先被创建的对象, 其析构函数是最后执行的
// 创建一个静态对象, 在程序退出时才调用这个静态对象的析构函数( 在析构函数中, 我们调用内存泄漏检测函数 ), 
// 这就得保证, 我们的静态对象必须先于被测文件的静态对象创建(如果有), 这样我们的静态对象才会最后一个析构(必须保证最后一个析构, 
// 以免发生问题( 如: 假设被测文件也有一个静态对象, 且静态对象申请了空间,  被测文件的静态对象在我们的静态对象析构后析构, 
// 这样内存泄漏检测就会不准确 )), 所以被测文件必须第一个包含我们的 LeakDetector.h 头文件保证我们的静态对象第一个创建
void LeakDetector::_LeakDetector( ){
	...
	while ( (NULL != ptr) && (&memoryListHead != ptr) ){
		...
		if ( NULL != ptr->_file )
			std::cout << " 位于 " << ptr->_file << " 第 " << ptr->_line << " 行"<< std::endl;
		else
